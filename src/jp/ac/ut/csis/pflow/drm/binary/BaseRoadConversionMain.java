...
public class BaseRoadConversionMain {
    public static void main(String[] args) {
        File drm21dir = new File(args[0]);
        File drm22dir = new File(args[1]);
    ...
    protected void invoke(File drm21dir, File drm22dir, File linkFile, File nodeFile) {
        Map<String, String> bounds = this.loadBoundaryNodes(drm21dir);
        File[] files = drm22dir.listFiles(new FileFilter(){
        ...
        this.exportNode(drm21dir, nodeFile, nodelist);
    ...
    private List<LonLat> composeGeometry(Mesh mesh, String[] tokens, BufferedReader br) throws IOException {
        ...
        int pnum = Integer.parseInt(tokens[43]);
        StrTokenizer st = StrTokenizer.getTSVInstance();
        int i = 0;
        while (i < pnum) {
            int n = i % 16;
            double x = rect.getMinX() + ((RectangularShape)rect).getWidth() * (double)Integer.parseInt(tokens[44 + n * 2]) / 10000.0;
            double y = rect.getMinY() + ((RectangularShape)rect).getHeight() * (double)Integer.parseInt(tokens[44 + (n * 2 + 1)]) / 10000.0;
            points.add(new LonLat(x, y));
            if ((i + 1) % 16 == 0 && i + 1 < pnum) {
    ...
    private String composeLinkOutputHeader() {
        ...
        output.addAll(ParseDRM22.COLUMNS.subList(0, 43));
        output.addAll(ParseDRM22.COLUMNS.subList(76, 79));
    ...
    private String composeLinkOutputString(String meshcode, String[] tokens, int nodeNo01, int nodeNo02, List<LonLat> geom) {
        Object[] tokens00 = (String[])ArrayUtils.subarray((Object[])tokens, (int)0, (int)43);
        Object[] tokens01 = (String[])ArrayUtils.subarray((Object[])tokens, (int)76, (int)79);
    ...
    private Map<String, String> loadBoundaryNodes(File drm21dir) {
        LinkedHashMap<String, String> nodemap = new LinkedHashMap<String, String>();
        File[] files = drm21dir.listFiles(new FileFilter(){
        ...
        System.out.println("complete loading DRM21");
        return nodemap;
    }

    private void exportNode(File drm21dir, File nodefile, Map<String, Integer> nodelist) {
        File[] files = drm21dir.listFiles(new FileFilter(){
