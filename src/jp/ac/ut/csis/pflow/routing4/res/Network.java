...
public class Network {
    ...
    public Network(boolean makeNodeIndex, boolean makeLinkIndex) {
        this._nodeIndex = makeNodeIndex ? new STRtree() : null;
        this._linkIndex = makeLinkIndex ? new STRtree() : null;
    }
    ...
    private void addIndex(Node node) {
        if (this._nodeIndex != null && TrajectoryUtils.validateLonLat(node)) {
    ...
    public List<Node> queryNode(double x0, double y0, double x1, double y1) {
        if (this._nodeIndex == null) {
            ...
            Polygon bounds = GeometryUtils.createPolygon((ILonLat[])new LonLat[]{new LonLat(x0, y0), new LonLat(x0, y1), new LonLat(x1, y1), new LonLat(x1, y0), new LonLat(x0, y0)});
    ...
    public void clear() {
        ...
        this._nodes.clear();
        if (this._nodeIndex != null) {
            this._nodeIndex = new STRtree();
        }
        if (this._linkIndex != null) {
            this._linkIndex = new STRtree();
        }
    }
